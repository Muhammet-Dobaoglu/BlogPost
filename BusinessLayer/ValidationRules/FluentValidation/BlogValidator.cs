...
namespace BusinessLayer.ValidationRules.FluentValidation
{
	public class BlogValidator : AbstractValidator<Blog>
	{
		public BlogValidator()
		{
			...
			RuleFor(x => x.BlogThumbnailImage).NotEmpty().WithMessage("Lütfen Blog Dosya Yolunu Boş Bırakmayınız!");
			RuleFor(x => x.BlogTitle).MaximumLength(150).WithMessage("Lütfen 150 Karakter'den daha az Veri Girişi Yapınız!");
			RuleFor(x => x.BlogContent).MinimumLength(30).WithMessage("Lütfen {MinLength} Karakter'den daha fazla Veri Girişi Yapınız!");
		}
	}
}